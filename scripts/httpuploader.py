...
def get_cli_arguments(argv):
    ...
    parser.add_argument(
        "--cert",
        metavar="CERT",
        help="file to read certificate configuration from",
    )

    parser.add_argument(
        "--key",
        metavar="KEY",
        help="file to read private key configuration from",
    )

    parser.add_argument(
        "--keypass",
        metavar="KEYPASS",
        help="file to read private key passphrase from",
    )
...
def main(argv=None):
    ...
    with make_server("", port, ul_serve, server_class=MTServer) as srv:
        if args.cert and args.key:
            if not args.keypass:
                args.keypass = None
            ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
            ctx.load_cert_chain(certfile=args.cert, keyfile=args.key, password=args.keypass)
